// 全局接口
var api_url="https://homeplus.fun" //线上接口
// var api_url="http://192.168.0.110:8080" //本地接口
// var api_url="https://47.100.171.118:8080"

// 全局定义跳转链接
// var api_jump="http://127.0.0.1:5500" //本地跳转链接
var api_jump="https://homeplus.fun/static/membershipPack" //线上跳转链接
...
　　return Y+M+D;//2019.11.06
}

// 时间戳转年月日时分秒 2019-11-06 12:12:12
function timestampToTimeTwo(timestamp) {
...
// 时间戳转年月日时分秒 2019/11/06 12:12:12
function timestampToTimeThree(timestamp) {
    　  var date = new Date(parseInt(timestamp));//时间戳为10位需*1000，时间戳为13位的话不需乘1000
        var Y = date.getFullYear() + '/';
        var M = (date.getMonth() + 1 < 10 ? '0' + (date.getMonth() + 1) : date.getMonth() + 1) + '/';
        var D =(date.getDate() < 10 ? '0'+date.getDate() : date.getDate()) + ' ';
...
// 时间戳转年月日 2019-11-06
function timestampToTimeFour(timestamp) {
　　var date = new Date(timestamp);
　　Y = date.getFullYear() + '-';
　　M = (date.getMonth()+1 < 10 ? '0'+(date.getMonth()+1) : date.getMonth()+1) + '-';
　　D = (date.getDate() < 10 ? '0'+date.getDate() : date.getDate())
